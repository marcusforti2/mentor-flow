...
export function PipelineStageEditor({ tenantId }: PipelineStageEditorProps) {
  const { toast } = useToast();
  const { stages: currentStages, isLoading, isCustom, reload, DEFAULT_STAGES } = usePipelineStages(tenantId);
  ...
  const handleSave = async () => {
    ...
    setIsSaving(true);
    try {
      // Delete existing custom stages
      await supabase.from("crm_pipeline_stages").delete().eq("tenant_id", tenantId);

      // Check if stages match default — if so, don't save (use fallback)
      const isDefault =
    ...
    <Card>
      <CardHeader>
        <div className="flex items-center justify-between">
          <div>
            <CardTitle className="text-lg">Configurar Pipeline</CardTitle>
            <p className="text-sm text-muted-foreground mt-1">
              Defina os estágios do pipeline que seus mentorados usarão. Arraste para reordenar.
            </p>
          </div>
