...
export default function MeusArquivos() {
  ...
  const stats = {
    total: files.length,
    files: files.filter(f => f.file_type === 'file').length,
    ...
    <div className="container max-w-4xl mx-auto p-6 space-y-6">
      ...
      <div className="text-center mb-8">
        ...
        <p className="text-muted-foreground mt-2">
          Arquivos e materiais compartilhados pelo seu mentor
        </p>
      </div>
      ...
      <div className="grid grid-cols-2 md:grid-cols-5 gap-3">
        ...
        <Card className="glass-card">
          <CardContent className="p-4 text-center">
            <p className="text-2xl font-bold text-purple-500">{stats.images}</p>
            <p className="text-xs text-muted-foreground">Imagens</p>
          </CardContent>
        </Card>
        <Card className="glass-card">
          <CardContent className="p-4 text-center">
            <p className="text-2xl font-bold text-blue-500">{stats.links}</p>
            <p className="text-xs text-muted-foreground">Links</p>
          </CardContent>
        </Card>
        <Card className="glass-card">
          <CardContent className="p-4 text-center">
            <p className="text-2xl font-bold text-yellow-500">{stats.notes}</p>
          ...
          {filteredFiles.length === 0 ? (
            <div className="text-center py-12">
              ...
              <p className="text-muted-foreground">
                Quando seu mentor compartilhar arquivos, eles aparecerão aqui.
              </p>
            </div>
          ) : (
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <div className="flex items-center gap-3 mt-1 text-xs text-muted-foreground">
                      ...
                      <Badge variant="outline" className="text-xs">
                        {file.file_type === 'file' && 'Arquivo'}
                        {file.file_type === 'image' && 'Imagem'}
                        {file.file_type === 'link' && 'Link'}
                        {file.file_type === 'note' && 'Nota'}
                      </Badge>
                    </div>
                  </div>
          ...
          <div className="mt-4">
            ...
            {selectedNote?.created_at && (
              <p className="text-xs text-muted-foreground mt-4">
                Criado em {format(new Date(selectedNote.created_at), "dd 'de' MMMM 'de' yyyy 'às' HH:mm", { locale: ptBR })}
              </p>
            )}
          </div>
        </DialogContent>
