...
const handler = async (req: Request): Promise<Response> => {
  ...
  try {
    ...
    if (audienceType === 'specific' && audienceMembershipIds.length > 0) {
      const { data, error } = await supabase.from('memberships').select('id, user_id').in('id', audienceMembershipIds).eq('status', 'active');
      if (error) throw error;
      memberships = data || [];
    } else {
      const { data, error } = await supabase.from('memberships').select('id, user_id').eq('tenant_id', tenantId).eq('role', 'mentee').eq('status', 'active');
      if (error) throw error;
      memberships = data || [];
    }

    if (memberships.length === 0) return new Response(JSON.stringify({ error: "Nenhum destinatário encontrado" }), { status: 400, headers: { "Content-Type": "application/json", ...corsHeaders } });
    ...
    if (ownerMembershipId) {
      const { data: ownerMembership } = await supabase.from('memberships').select('user_id').eq('id', ownerMembershipId).single();
      if (ownerMembership) {
        const { data: mentorProfile } = await supabase.from('profiles').select('full_name').eq('user_id', ownerMembership.user_id).single();
        if (mentorProfile?.full_name) mentorName = mentorProfile.full_name;
      }
    }

    const membershipIds = memberships.map(m => m.id);
    const { data: menteeProfiles } = await supabase.from('mentee_profiles').select('membership_id, business_name').in('membership_id', membershipIds);
    const businessMap = new Map(menteeProfiles?.map(bp => [bp.membership_id, bp]) || []);

    const { data: membershipDetails } = await supabase.from('memberships').select('id, created_at').in('id', membershipIds);
    ...
    const recipients = memberships.map(m => {
      ...
      return { membershipId: m.id, userId: m.user_id, email: profile?.email, fullName: profile?.full_name || 'Mentorado', businessName: biz?.business_name || '', diasNaJornada };
    }).filter(r => r.email);

    if (recipients.length === 0) return new Response(JSON.stringify({ error: "Nenhum destinatário com email encontrado" }), { status: 400, headers: { "Content-Type": "application/json", ...corsHeaders } });
    ...
    if (emailNodes.length === 0) return new Response(JSON.stringify({ error: "Nenhum nó de email no fluxo" }), { status: 400, headers: { "Content-Type": "application/json", ...corsHeaders } });

    const { data: certificates } = await supabase.from('certificates').select('membership_id').in('membership_id', membershipIds);
    const trailCountMap = new Map<string, number>();
    if (certificates) for (const cert of certificates) { if (cert.membership_id) trailCountMap.set(cert.membership_id, (trailCountMap.get(cert.membership_id) || 0) + 1); }
    ...
    const replaceVars = (text: string, recipient: typeof recipients[0]) => {
      ...
      return text.replace(/\{\{nome\}\}/g, recipient.fullName).replace(/\{\{email\}\}/g, recipient.email || '').replace(/\{\{business_name\}\}/g, recipient.businessName).replace(/\{\{mentor_name\}\}/g, mentorName).replace(/\{\{dias_na_jornada\}\}/g, String(recipient.diasNaJornada)).replace(/\{\{trilhas_concluidas\}\}/g, String(trilhasConcluidas));
    };
  ...
  <div class="email-container">
    ...
    <div class="footer">
      ...
      <p>© ${new Date().getFullYear()} ${branding.name}. Todos os direitos reservados.</p>
        ...
        try {
          await resend.emails.send({ from: branding.fromEmail, to: [recipient.email!], subject: personalizedSubject, html: htmlContent });
          successCount++;
        } catch (err: any) {
          errorCount++;
      ...
      try { await supabase.from('email_flow_executions').insert({ flow_id: flowId, membership_id: recipient.membershipId, status: 'completed', started_at: new Date().toISOString(), completed_at: new Date().toISOString() }); } catch (e) { console.error('Failed to record execution:', e); }
    }

    if (!immediate) await supabase.from('email_flows').update({ is_active: true }).eq('id', flowId);

    const totalEmails = recipients.length * emailNodes.length;
    const message = errorCount > 0 ? `${successCount} de ${totalEmails} email(s) enviados. ${errorCount} falharam.` : `${successCount} email(s) enviados para ${recipients.length} mentorado(s) com sucesso!`;

    return new Response(JSON.stringify({ success: errorCount === 0, message, stats: { total: totalEmails, sent: successCount, failed: errorCount }, errors: errors.length > 0 ? errors : undefined }), { status: 200, headers: { "Content-Type": "application/json", ...corsHeaders } });
  } catch (error: any) {
